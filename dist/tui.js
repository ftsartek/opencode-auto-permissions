...
function toolParts(message) {
  if (!isRecord(message))
    return [];
  const parts = message.type === "assistant" && Array.isArray(message.content) ? message.content : isRecord(message.info) && message.info.role === "assistant" && Array.isArray(message.parts) ? message.parts : [];
  return parts.filter((part) => isRecord(part) && part.type === "tool");
}
function questionEntries(message) {
  return toolParts(message).flatMap((part) => {
    if (part.tool !== "question" || !isRecord(part.state) || part.state.status !== "completed")
      return [];
    const questions = isRecord(part.state.input) && Array.isArray(part.state.input.questions) ? part.state.input.questions : [];
    const answers = isRecord(part.state.metadata) && Array.isArray(part.state.metadata.answers) ? part.state.metadata.answers : [];
